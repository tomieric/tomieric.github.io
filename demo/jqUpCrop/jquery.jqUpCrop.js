				...
				if (e[0]) {
					if (e[0] == "*") {
						...
						c("#" + j.queueID).find(".uploadify-queue-item").each(function() {
							...
							c(this).find(".data").removeClass("data").html(" - "+j.cancelText);
						...
						for (var m = 0; m < e.length; m++) {
							i.cancelUpload(e[m]);
							c("#" + e[m]).find(".data").removeClass("data").html(" - "+j.cancelText);
	...
	var a = {
		...
		onUploadSuccess: function(f, h, d) {
			...
			if (c.inArray("onUploadSuccess", g.overrideEvents) < 0) {
				c("#" + f.id).find(".data").html(" - "+g.completeText)
			}
...
/*
* copyright by http://www.fenxiangyuan.com
* author : tomieric
*/

(function(){
	$.fn.jqUpCrop = function(config){
		var defaults ={
			buttonText : "上传图片",
			completeText : "已完成！",
			cancelText : "已取消！",
			...
			bigImg : "#bigImg",
			smallImg : "#smallImg",
			smallImgWp: "#preview_box",
			upfolder : "uploads/",
			submitBtn : "#crop_submit",
			isCrop : true,
			cropCfg : {
				aspectRatio:1,
				onChange:showCoords,
			...
			cropUplodUrl : "crop.php"
		};
		
		var cfg = $.extend(defaults,config);
		
		var that = $(this), bigImg = $(cfg.bigImg), smallImg = $(cfg.smallImg), smallImgWapper = $(cfg.smallImgWp);
		
		if(smallImg) smallImg.hide();	
		
		var def_w = Math.floor(smallImgWapper.width(),0), def_h = Math.floor(smallImgWapper.height(),0),big_w = bigImg.width();
		
		//是否启用截图，是则配置初始化参数
		if(cfg.isCrop){
			//截图位置及大小数据存放表单
			var cropForm = $('<form action="'+cfg.cropUplodUrl+'" method="post" id="crop_form" style="display: none">\
								<input type="hidden" id="crop_x" name="x" value="" />\
								<input type="hidden" id="crop_y" name="y" value="" />\
								<input type="hidden" id="crop_w" name="w" value="'+def_w+'"  />\
								<input type="hidden" id="crop_h" name="h" value="'+def_h+'" />\
								<input type="hidden" id="crop_img" name="imgurl" />\
							</form>');	
			/*var cropForm = $('<form action="'+cfg.cropUplodUrl+'" method="post" id="crop_form" style="display: ">\
								<input type="text" id="crop_x" name="x" value="" />\
								<input type="text" id="crop_y" name="y" value="" />\
								<input type="text" id="crop_w" name="w" value="'+def_w+'"  />\
								<input type="text" id="crop_h" name="h" value="'+def_h+'" />\
								<input type="text" id="crop_img" name="imgurl" />\
							</form>');			*/		
			cropForm.appendTo($("body"));
			
			//表单提交
			$(cfg.submitBtn).live("click",function(){
					if(!$("#crop_img").val()){
						alert("请先上传图片！");
						return;
					}
			...
			//为uploadify 上传成功 添加回调函数
			cfg.onUploadSuccess = function(file,data,res){
					//显示大图
					bigImg.attr("src",cfg.upfolder+file.name);
					//预览图
					smallImg.attr("src",cfg.upfolder+file.name).fadeIn();
					//处理图片的信息
					$("#crop_img").val(cfg.upfolder+file.name);
					//使用Jcrop插件
					bigImg.Jcrop(cfg.cropCfg);
			}
		}
		//初始化uploadify插件
		that.uploadify(cfg);
		...
		function showCoords(obj){
			...
			//获取图像缩放比例 
			//var scale = bigImg[0].width / bigImg.width();  only FF
			/* not work in chorme
			var originalIamge = new Image();
			originalIamge.src = bigImg[0].src;
			 if (originalIamge.complete) {
				 var scale = originalIamge.width / Math.floor(bigImg.width());
			 }
			 
			 if(!originalIamge.width) return; 
			 
			 console.log(originalIamge.width);*/
			
			 var scale = 1;
			 imgReady(bigImg[0].src,function(){
				scale = this.width / big_w;
				$("#crop_x").val(Math.floor(obj.x * scale));
				$("#crop_y").val(Math.floor(obj.y * scale));
				$("#crop_w").val(Math.floor(obj.w * scale));
				$("#crop_h").val(Math.floor(obj.h * scale));
			 });
			 
			if(parseInt(obj.w) > 0){
				//计算预览区域图片缩放的比例，通过计算显示区域的宽度(与高度)与剪裁的宽度(与高度)之比得到
				var rx = smallImgWapper.width() / obj.w; 
				var ry = smallImgWapper.height() / obj.h;
				
				//通过比例值控制图片的样式与显示
				smallImg.css({
					width:Math.round(rx * bigImg.width(),0) + "px",	//预览图片宽度为计算比例值与原图片宽度的乘积
					height:Math.round(rx * bigImg.height(),0) + "px",	//预览图片高度为计算比例值与原图片高度的乘积
					marginLeft:"-" + Math.round(rx * obj.x,0) + "px",
					marginTop:"-" + Math.round(ry * obj.y,0) + "px"
				});
			}
		}
